...
int main(int argc, char** argv)
{
  ...
  amcl_node_ptr.reset(new AmclNode());  // 初始化
        ...
        first_reconfigure_call_(true)
{
  ...
  // 发布话题
  pose_pub_ = nh_.advertise<geometry_msgs::PoseWithCovarianceStamped>("amcl_pose", 2, true); //定位
  particlecloud_pub_ = nh_.advertise<geometry_msgs::PoseArray>("particlecloud", 2, true);//粒子群中每个位姿
  // 发布服务
  global_loc_srv_ = nh_.advertiseService("global_localization", &AmclNode::globalLocalizationCallback,this);  // 重置粒子分布，重新均匀分布，铺满地图
  nomotion_update_srv_= nh_.advertiseService("request_nomotion_update", &AmclNode::nomotionUpdateCallback, this); // 强制更新
  set_map_srv_= nh_.advertiseService("set_map", &AmclNode::setMapCallback, this); //也是调用handleMapMessage 拿到地图数据
  ...
  laser_scan_filter_ = 
          new tf2_ros::MessageFilter<sensor_msgs::LaserScan>(*laser_scan_sub_,
                                                             *tf_,
                                                             ...
                                                             100,
                                                             nh_);
  // 雷达回调的核心函数：laserReceived，所有算法的处理！！！                                                          
  laser_scan_filter_->registerCallback(boost::bind(&AmclNode::laserReceived,        // core!!!!!!!!!
                                                   this, _1));
  //初始位姿的tf数据  接受来自global_frame_id_的数据
  initial_pose_sub_ = nh_.subscribe("initialpose", 2, &AmclNode::initialPoseReceived, this);

  // 函数： mapReceived、requestMap、setMapCallback
  // 核心调用handleMapMessage 拿到地图数据
  if(use_map_topic_) {  // 以话题形式接受map信息还是服务形式，一般选用服务直接去加载地图文件
    map_sub_ = nh_.subscribe("map", 1, &AmclNode::mapReceived, this);
    ROS_INFO("Subscribed to map topic.");
  } else {
    requestMap();   // 请求服务static_server提供map，调用handleMapMessage处理地图信息，然后获取初始位姿、初始化粒子、里程计
  }
...
AmclNode::requestMap()
{
  ...
  handleMapMessage( resp.map ); //接受地图数据
}
...
AmclNode::handleMapMessage(const nav_msgs::OccupancyGrid& msg)
{
  ...
  // Initialize the filter
  updatePoseFromServer();   // 初始化粒子位姿和方差
  pf_vector_t pf_init_pose_mean = pf_vector_zero();
...
void
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)     // 相当核心！！
{
  // 获取雷达坐标系和当前帧时间
  ...
  else if(pf_init_ && lasers_update_[laser_index])  // 如果已经初始化，并需要更新，则更新粒子运动
  {
    //printf("pose\n");
  ...
  // If the robot has moved, update the filter
  if(lasers_update_[laser_index])   // 需要更新
  {
    // 雷达数据格式转换为AMCLLaserData
    AMCLLaserData ldata;
    ...
    if(laser_max_range_ > 0.0)
      ldata.range_max = std::min(laser_scan->range_max, (float)laser_max_range_); //限制激光最远距离
    else
      ldata.range_max = laser_scan->range_max;
    double range_min;
    if(laser_min_range_ > 0.0)
      range_min = std::max(laser_scan->range_min, (float)laser_min_range_);   //限制激光最近距离
    else
    ...
    for(int i=0;i<ldata.range_count;i++)
    {
      ...
      if(laser_scan->ranges[i] <= range_min)    // 比最小值还小，就把最大的当做最小值，
        ldata.ranges[i][0] = ldata.range_max;
      else if(laser_scan->ranges[i] > ldata.range_max)  // 限制到最远激光
    ...
    if (!m_force_update)
    {
      ...
      particlecloud_pub_.publish(cloud_msg);    // 将新粒子发布到全局坐标系下
    }
  }
